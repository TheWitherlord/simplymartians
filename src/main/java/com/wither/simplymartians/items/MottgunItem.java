...
public class MottgunItem extends BowItem {
	
	private static final int COOLDOWN = 15;

	public MottgunItem(final Item.Properties properties) {
		super(properties);
	}
	...
	protected Projectile createProjectile(final Level level, final LivingEntity shooter, final ItemStack projectile, final ItemStack weapon, final boolean isFullPower) {
		return new MottLaser(level, shooter);
	}
	 ...
	 @Override
	    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
	        ...
	        if (!pLevel.isClientSide) {
	            ...
	            for (int i = 0; i < 7; i++) {
	            	MottLaser pellet = new MottLaser(pLevel, pPlayer);
