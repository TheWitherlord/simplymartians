...
@OnlyIn(Dist.CLIENT)
public class FluxWaveRenderer extends EntityRenderer<FluxWave> {
	private static final ResourceLocation WAVE_LOCATION = ResourceLocation
			.fromNamespaceAndPath(SimplyMartians.MODID, "textures/entity/flux_wave.png");
	private static final RenderType RENDER_TYPE = RenderType.entityTranslucentEmissive(WAVE_LOCATION);

	public FluxWaveRenderer(EntityRendererProvider.Context p_173962_) {
		super(p_173962_);
	}

	protected int getBlockLightLevel(ZapBolt p_114087_, BlockPos p_114088_) {
	      return 15;
	   }

	public void render(FluxWave pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
        pPoseStack.scale(2.0F, 2.0F, 2.0F);
        pPoseStack.mulPose(this.entityRenderDispatcher.cameraOrientation());
        PoseStack.Pose posestack$pose = pPoseStack.last();
        VertexConsumer vertexconsumer = pBuffer.getBuffer(RENDER_TYPE);
        pPoseStack.mulPose(Axis.XP.rotationDegrees(105.0F));
	...
	public ResourceLocation getTextureLocation(FluxWave p_114078_) {
		return WAVE_LOCATION;
	}
}