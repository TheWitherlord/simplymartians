...
public class MartianEngineerModel<T extends MartianEngineerEntity> extends EntityModel<T> implements ArmedModel, HeadedModel{
		// This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
			 ResourceLocation.fromNamespaceAndPath(SimplyMartians.MODID, "martian_engineer"), "main");	
	private final ModelPart head;
		...
		public MartianEngineerModel(ModelPart root) {
		...
		public static LayerDefinition createBodyLayer() {
			...
			PartDefinition headwear = partdefinition.addOrReplaceChild("headwear", CubeListBuilder.create().texOffs(0, 62).addBox(-5.0F, -14.0F, -6.0F, 10.0F, 16.0F, 12.0F, new CubeDeformation(0.5F)), PartPose.offset(0.0F, 0.0F, 0.0F));
		...
		public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
			 this.head.yRot = pNetHeadYaw * ((float)Math.PI / 180F);
		      ...
		      this.headwear.yRot = pNetHeadYaw * ((float)Math.PI / 180F);
		      this.headwear.xRot = pHeadPitch * ((float)Math.PI / 180F);
			 this.right_leg.xRot = -1.5F * Mth.triangleWave(pLimbSwing, 13.0F) * pLimbSwingAmount;
		      ...
		      if (pEntity.getItemInHand(InteractionHand.MAIN_HAND).getItem() == InitItem.PLASMACUTTER.get()) {
