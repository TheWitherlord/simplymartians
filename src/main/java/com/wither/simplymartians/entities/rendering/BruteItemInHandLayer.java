...
@OnlyIn(Dist.CLIENT)
public class BruteItemInHandLayer<T extends LivingEntity, M extends EntityModel<T> & ArmedModel> extends RenderLayer<T, M> {
    private final ItemInHandRenderer itemInHandRenderer;

    public BruteItemInHandLayer(RenderLayerParent<T, M> pRenderer, ItemInHandRenderer pItemInHandRenderer) {
        super(pRenderer);
    ...
    protected void renderArmWithItem(
        ...
        if (!pItemStack.isEmpty()) {
            ...
            pPoseStack.translate((float)(flag ? -1 : 1) / 8.0F, 0.035F, -0.725F);
